...
local maps = {
	...
	['vanilla'] = {2,11,12,19,22,24,26,40,44,45,49,57,67,69,70,71,73,74,75,77,79,80,85,86,119,123,127,138,142,145, 149,150,172,173,174,7470754,7470763,7470774,7470782,7470797,7697587,7767228,7815228,7815665,7815369,7815133,7815352,7767311,7767308,7815151,7815374,7767224,7767300,7821451,7821747},
	['bootcamp'] = {'#13', '#3'},
	['defilante'] = {'#18'},
	['racing'] = {'#17'},
	['custom1'] = {},
...
checkPlayers = function()
	for k, v in next, playerData do
		...
		if room.settings.noSouris then
			if k:sub(1,1) == "*" then
				tfm.exec.killPlayer(k)
			end
		end
...
commands.six = {
	...
	event = function(playerData, cmd_args)
		local target = cmd_args[2] and cmd_args[2] or playerData.name
		local data = room.playerData[target] or nil
		local clearImage = function(player)
			if player == "all" then
				for k, v in next, room.playerData do
					if v.inGame then
						if #v.images > 0 then
...
eventNewGame = function()
	...
	if not tfm.get.room.xmlMapInfo then
		print("<VI>[eventNewGame] <N>-> Could not find current map code, switching to hub")
		room.currentMap = hub
	else
